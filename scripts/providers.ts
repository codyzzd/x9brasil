...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      const delay = initialDelay * Math.pow(2, attempt);
      console.warn(`  ⚠️ Falha de rede (${err.message}). Tentativa ${attempt + 1}/${maxRetries}. Aguardando ${delay / 1000}s...`);
      await sleep(delay);
    }
  }
...
class LMStudioProvider implements LLMProvider {
  ...
  async generateContent(prompt: string, options: { model: string; temperature?: number; responseMimeType?: string }): Promise<string> {
    ...
    const json: any = await res.json();
    if (json.error) throw new Error(`Erro do LM Studio: ${json.error.message || JSON.stringify(json.error)}`);
    const text = json.choices?.[0]?.message?.content;
    if (!text) {
...
export function getProvider(id: ProviderId, config?: ProviderConfig): LLMProvider {
  switch (id) {
    ...
    case "openai": return new OpenAICompatibleProvider(config?.baseUrl || "https://api.deepseek.com");
...
export async function interactiveSelect(
  ...
  // API key interativa se não encontrar na env
  if (provider.id === "openai" && !env.OPENAI_API_KEY) {
    const key = (await rl.question("OPENAI_API_KEY (opcional, ou exporte no terminal): ")).trim();
